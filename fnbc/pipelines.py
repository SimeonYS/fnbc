...
class FnbcPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('fnbc.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `fnbc`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM fnbc WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO fnbc (date, title, link, content)"
